...
## 九、线上复盘：为什么候选命中 0.980 仍然拒答

一次实际提问是“Celery、MySQL 和 Redis 如何分工？”。界面找到了本文，候选来源旁显示 `0.980`，但回答区仍提示未通过引用检查。表面上看，这是高相似度与拒答互相矛盾；检查链路后发现，它们属于两个不同阶段。

`0.980` 是检索阶段的候选命中分，不是答案有 98% 的可信度。为了让中英文混合的短问题稳定找到文章，检索器在正文同时包含多个英文关键词时会给出固定的关键词匹配分。这个分数只回答“哪篇资料更可能相关”，不判断模型最终写出的每句话是否带有有效来源。

检索通过后，生成阶段还要求模型返回“结论文本 + 引用编号”的结构化结果。免费模型可能把 JSON 包在代码块里、把引用数组写成字符串，或直接返回已经带 `[1]` 的正文。旧解析器只接受一种严格 JSON 形状，这些有依据但格式略有差异的输出会被当成失败；重试后，界面又把结构解析失败、缺少引用和引用越界统一描述为“逐段引用一致性失败”，于是掩盖了真正原因。

这次修正做了三件事：

1. 界面把来源旁的数字明确标成“候选命中”，与回答的引用复核分分开。
2. 生成解析器兼容代码块 JSON、字符串形式的引用、正文内引用和逐段带引用的纯文本；问题同时点名多个技术时，还会检查答案是否逐一覆盖，遗漏时自动要求模型补全。
3. 拒答结果区分“没有检索到足够资料”和“已经命中资料，但模型输出格式无效”，便于继续定位模型或索引问题。

因此，处理这类问题时不能只调低阈值。应依次检查检索是否命中、生成输出能否解析、引用编号是否存在且没有越界，以及引用内容是否真的支持结论。高检索分只能证明第一步通过。
